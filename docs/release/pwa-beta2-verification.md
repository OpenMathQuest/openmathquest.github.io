...
and true silent Beta 1 windows alike. Exact-shell proof and known obsolete-cache
cleanup remain mandatory, but neither grants authority to force a window to a
new URL. The focused worker regression exercises all four client classes and
...
current page may still coordinate its own exact, responsive lifecycle through
the reviewed safe-boundary controller-change path. It tells the grown-up that
other open tabs must be reloaded or reopened manually. The one-use
