...
against an accelerated renderer clock. The probe starts when the primary
shipped frame becomes available and independently bounds service-worker
registration readiness, controller acquisition, and the readiness-message
response. `BR-24` records exact structured stage and timeout details on a
failure, cleans up its listener or message port, and then permits the remaining
visual/profile checks to run. The browser runner's independent wall-clock limit
remains a separate whole-audit fail-closed boundary.
...
On 2026-07-30 the replacement controller completed the then-current locally
installed Edge matrix with 71 of 71 browser groups passing, zero unexpected
requests, an observed completion fence, a clean CDP browser close, and no
forced cleanup. The current Beta 4 oracle contains 72 browser groups, so that
historical local result validates the controller mechanism but does not
substitute for current hosted evidence. Two consecutive 72-result
GitHub-hosted passes on the exact same commit are required before this
remediation can support release clearance.
...
The page observes the exact waiting worker in the tab whose grown-up selected
Apply. That tab performs the save-before-reload interlock exactly once after
activation. Any independent `controllerchange` refreshes readiness and offers
a deliberate Reload action; it never automatically reloads at Home or the
Grown-ups corner and never discards an unfinished nickname or answer. Candidate discovery, caching,
...
Activation cleanup is limited to the candidate's disposable staging cache and
begins only after one more full exact-cache proof at the activation event. A
mutation after the waiting-worker handshake therefore rejects activation and
preserves the prior cache. Exact activation does not call `clients.claim()` or
delete any prior release cache, because an older open tab may still depend on
that worker and those offline bytes. Staging cleanup failure is nonfatal and
does not touch `localStorage`.

An activated worker never probes, classifies, or navigates retained windows.
Silence, a timeout, a malformed message, and cache lineage cannot establish
which release a loaded document is running. This prohibition covers responsive
current windows, suspended or unknown post-Beta-1 windows, malformed clients,
and true silent Beta 1 windows alike. Exact-shell proof remains mandatory, and
prior release caches remain available to any older open tab until ordinary
browser storage eviction. Neither cache presence nor cleanup grants authority
to force a window to a new URL. The focused worker regression exercises all four client classes and
requires zero version challenges and zero automatic navigations, both with a
Beta-1-only cache lineage and with Beta 1 plus a later obsolete cache.

A grown-up recovers a retained Beta 1 window by deliberately reloading it or by
closing and reopening it. The verified current worker then serves the exact
shell and the existing transactional Beta 1 migration copies only validated
local progress; the worker itself cannot access or mutate that progress. The
tab whose grown-up selected Apply may coordinate its own exact lifecycle
through the waiting worker's activated state. It tells the grown-up that other
open tabs must be reloaded or reopened manually. The one-use
`legacy-recovery=beta1` parser remains only for backward compatibility with a
URL emitted by an already-installed older worker and is not generated by this
release.
...
nonredirected response with the registered MIME and hash. An already-loaded
legacy document can remain visible until its grown-up chooses to reload or
reopen it. The exact retained-client canary must independently prove zero
automatic navigation followed by one deliberate reload; any selected
physical-device observation remains separate evidence rather than being
inferred from the focused worker harness.
...
The focused development entry point invokes both commands while shell work is
in progress. After the manifest and worker are finalized, commit and freeze the
exact public candidate. The release-only Windows entry point, `audit.bat`, then
invokes both commands again inside the single complete pre-publication
certification cycle. A stale binding or failed PWA effect therefore stops the
release; focused commands remain development feedback only.
...
Only after every listed shell byte is final, but before the immutable candidate
commit and payload are frozen, write the canonical manifest and worker binding:
...
verification fails. When the manifest hash changes, it changes only the exact
worker binding; it never converts a prior live cache into a deletion target.
Any product
...
No desktop emulation is real Apple-device evidence. For Beta 4, the owner
declined the offered physical-device matrix and six fresh antagonist-review
cycles; both remain exact `OPTIONAL_NOT_RUN`, and this release makes no
corresponding device or independent-review claim. For an eligible prerelease,
the host review remains
visibly `DEFERRED_PRERELEASE`, not passed or privacy-cleared; affirmative host
qualification becomes mandatory before stable release. Canary reconciliation,
application-level privacy, independent adjudication, finding disposition,
hosted-Windows evidence, pending-to-approved publication clearance, the
annotated release tag, and exact live-artifact reconciliation remain mandatory
as defined by `ios-ipados-pwa-beta2-plan.md`.
