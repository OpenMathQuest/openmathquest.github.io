...
outside the engine owns the Public Beta 9 PWA lifecycle.

## Frozen identities

- Release: `1.0.0-beta.9`
- Build: `math-quest-pwa-v1.0.0-beta.9`
- Reported logical cache identity: `math-quest-static-v1.0.0-beta.9`
- Physical cache storage:
  `math-quest-static-v1.0.0-beta.9-<release-manifest-sha256>`
...
Two worker scripts that accidentally reuse the same logical Beta 9 identity
...
Beta 9. Its full state must also pass the existing engine validator after only
...
Beta 1 bytes verbatim and start from the canonical fresh Beta 9 state; it must
...
outside the Beta 9 support claim; the explanation recommends another qualified
