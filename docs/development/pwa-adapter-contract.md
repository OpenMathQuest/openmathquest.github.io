...
outside the engine owns the Public Beta 8 PWA lifecycle.

## Frozen identities

- Release: `1.0.0-beta.8`
- Build: `math-quest-pwa-v1.0.0-beta.8`
- Reported logical cache identity: `math-quest-static-v1.0.0-beta.8`
- Physical cache storage:
  `math-quest-static-v1.0.0-beta.8-<release-manifest-sha256>`
...
Two worker scripts that accidentally reuse the same logical Beta 8 identity
...
Beta 8. Its full state must also pass the existing engine validator after only
...
Beta 1 bytes verbatim and start from the canonical fresh Beta 8 state; it must
...
outside the Beta 8 support claim; the explanation recommends another qualified
