...
export const TRUSTED_HTTPS_CANARY_TAG = "v1.0.0-beta.7";
...
export const RETAINED_BETA1_FRESH_START_NOTICE = "A Beta 1 save from the earlier curriculum remains stored separately on this device. Beta 7 starts fresh so old mastery is not applied to changed skills.";
...
export function parseTrustedHttpsCanaryEvidence(text, expected = {}) {
  ...
  issueIf(issues, value.intendedReleaseTag !== TRUSTED_HTTPS_CANARY_TAG, "intendedReleaseTag must be the Beta 7 tag");
  ...
  issueIf(issues, !/^math-quest-static-v1\.0\.0-beta\.7-[a-f0-9]{64}$/u.test(String(value.cacheProof?.physicalCacheName || "")) && !(failed && value.cacheProof?.physicalCacheName === null), "physical cache name must bind the logical name and detached manifest SHA-256");
  issueIf(issues, value.reconciliationState === "RECONCILED"
    && value.cacheProof?.physicalCacheName !== `math-quest-static-v1.0.0-beta.7-${value.runtimeIdentity?.candidateReleaseManifestSha256}`, "physical cache name must exactly bind the candidate release-manifest SHA-256");
  ...
  if (value.reconciliationState === "RECONCILED") {
    ...
    issueIf(issues, value.offlineProof?.responseFromServiceWorker !== true
      ...
      || value.offlineProof?.readinessRelease !== "1.0.0-beta.7"
      || value.offlineProof?.readinessBuildId !== "math-quest-pwa-v1.0.0-beta.7"
      || value.offlineProof?.readinessCacheIdentity !== "math-quest-static-v1.0.0-beta.7", "RECONCILED evidence requires a service-worker cold response with both server ports closed and exact readiness identity");
    issueIf(issues, value.progress?.protectedSha256 !== value.progress?.expectedFreshSha256
      ...
      || value.progress?.retainedNoticeSha256 !== RETAINED_BETA1_FRESH_START_NOTICE_SHA256, "RECONCILED evidence requires byte-identical retired Beta 1 retention, an independently fresh protected Beta 7 state, and the exact visible grown-up notice");
