...
const TUTORIAL_PROJECTION_VERSION = "tutorial-curriculum-projection-v1";
const TUTORIAL_FEATURE_SEED = 1297175628;
const TUTORIAL_FEATURE_ORDINALS = 32;
...
function collectTutorialVariantRows(engine, skill, { tier, representation, theme }, rows) {
...
function collectTutorialSkillRows(engine, skill, rows) {
  for (const tier of ["EASY", "HARD/TARGET"]) {
    for (const representation of ["CONCRETE", "PICTORIAL", "ABSTRACT"]) {
      for (const theme of ["ocean", "forest", "space"]) {
        collectTutorialVariantRows(engine, skill, { tier, representation, theme }, rows);
      }
    }
  }
}
...
export function tutorialFeatureInventory(engine) {
  ...
  const rows = new Map();
  for (const skill of engine.SKILLS) collectTutorialSkillRows(engine, skill, rows);
  return Object.freeze([...rows.values()].sort((left, right) => canonicalizeJson(left).localeCompare(canonicalizeJson(right), "en")));
}

function tutorialCurriculumProjection(curriculum) {
...
function validateTutorialAuthority(manifest, artDesign, issues) {
...
function validateTutorialCurriculum(manifest, curriculumArtifact, questionGeneratorContractVersion, issues) {
...
function validatePhaseCatalog(manifest, issues) {
  ...
  return expectedPhaseIds;
}

function validateAnchorCatalog(manifest, issues) {
...
function validateCueCatalog(manifest, issues) {
...
function validateFamilyPhases(family, { expectedPhaseIds, anchorSet, cueSet }, issues) {
...
function validateTutorialFamilies(manifest, context, issues) {
  ...
  for (const family of manifest.tutorialFamilies) validateFamilyPhases(family, context, issues);
  return familySet;
}

function validateMethodFamily(bindingRecord, familySet, issues) {
...
function validateMethodBindings(manifest, inputMethods, familySet, issues) {
  ...
  for (const bindingRecord of manifest.methodBindings) validateMethodFamily(bindingRecord, familySet, issues);
  return methodIds;
}

function validateProfileBindings(manifest, curriculumArtifact, familySet, issues) {
...
function validateTutorialFeatures(manifest, { featureInventory, methodIds, profileIds }, issues) {
...
function validateObligationTask(bindingRecord, taskTypesBySkill, issues) {
...
function validateTutorialObligations(manifest, projection, issues) {
  ...
  for (const bindingRecord of manifest.obligationBindings) validateObligationTask(bindingRecord, taskTypesBySkill, issues);
}

function validateTutorialResolution(manifest, issues) {
...
function validateTutorialStrings(manifest, childStringIds, issues) {
  if (childStringIds) {
...
export async function validateTutorialManifest(manifest, {
  ...
  validateTutorialAuthority(manifest, artDesign, issues);
  const projection = validateTutorialCurriculum(manifest, curriculumArtifact, questionGeneratorContractVersion, issues);
  const expectedPhaseIds = validatePhaseCatalog(manifest, issues);
  const anchorSet = validateAnchorCatalog(manifest, issues);
  const cueSet = validateCueCatalog(manifest, issues);
  const familySet = validateTutorialFamilies(manifest, { expectedPhaseIds, anchorSet, cueSet }, issues);
  const methodIds = validateMethodBindings(manifest, inputMethods, familySet, issues);
  const profileIds = validateProfileBindings(manifest, curriculumArtifact, familySet, issues);
  validateTutorialFeatures(manifest, { featureInventory, methodIds, profileIds }, issues);
  validateTutorialObligations(manifest, projection, issues);
  validateTutorialResolution(manifest, issues);
  validateTutorialStrings(manifest, childStringIds, issues);
