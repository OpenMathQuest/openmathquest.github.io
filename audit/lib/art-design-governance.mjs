...
const ART_DESIGN_DECISION_SCHEMA_PATH = new URL("../schemas/art-design-decision-register-v1.schema.json", import.meta.url);
const ART_ASSET_SCHEMA_PATH = new URL("../schemas/art-asset-register-v1.schema.json", import.meta.url);
const DESIGN_TOKENS_SCHEMA_PATH = new URL("../schemas/design-tokens-v1.schema.json", import.meta.url);
...
const TEXT_USAGE_CLASSES = new Set(["TEXT_OR_STROKE", "STATE_WITH_NON_COLOUR_CUE"]);
...
const validateArtAssetSchema = (value, schemaPath = ART_ASSET_SCHEMA_PATH) => validateSchema(value, schemaPath);
const validateDesignTokensSchema = (value, schemaPath = DESIGN_TOKENS_SCHEMA_PATH) => validateSchema(value, schemaPath);
...
function decisionIndexes(decisions) {
  return {
    sourceById: new Map(decisions.sourceDecisions.map((record) => [record.id, record])),
    rulesById: new Map(decisions.designRules.map((record) => [record.id, record])),
    workflowStepsById: new Map(decisions.constructionWorkflow.steps.map((record) => [record.id, record])),
  };
}

function validateSourceDecisionIdentity(record, expected, sourceBundle, issues) {
  ...
  if (record.sourcePath !== sourceBundle.decisionRegisterPath) issues.push(`${record.id} sourcePath does not match the source-bundle decision register path.`);
  if (record.sourceFileSha256 !== sourceBundle.decisionRegisterSha256) issues.push(`${record.id} sourceFileSha256 does not match the source-bundle decision register hash.`);
...
function validateDecisionIdentities(decisions, issues) {
  ...
  for (let index = 0; index < source.length; index += 1) {
    validateSourceDecisionIdentity(source[index], index + 1, decisions.sourceBundle, issues);
  }
}

function validateSourceProjection(record, rulesById, issues) {
...
function validateSourceRuleLinks(record, rulesById, issues) {
  ...
  validateSourceProjection(record, rulesById, issues);
}

function validateDesignRuleLinks(rule, sourceById, issues) {
...
function validateDecisionLinks(decisions, indexes, issues) {
  for (const record of decisions.sourceDecisions) validateSourceRuleLinks(record, indexes.rulesById, issues);
  const rules = decisions.designRules;
  for (const rule of rules) validateDesignRuleLinks(rule, indexes.sourceById, issues);
...
function validateImplementationBindings(record, indexes, issues) {
  if (!indexes.migrationIds.has(record.migrationId)) issues.push(`${record.id} references unknown migration ${record.migrationId}.`);
  for (const id of record.affectedSourceDecisionIds) if (!indexes.sourceById.has(id)) issues.push(`${record.id} references unknown source decision ${id}.`);
  for (const id of record.affectedDesignRuleIds) if (!indexes.rulesById.has(id)) issues.push(`${record.id} references unknown design rule ${id}.`);
  for (const proof of record.requiredProofs) if (!ART_PROOF_IDS.has(proof)) issues.push(`${record.id} has unknown proof ${proof}.`);
}

function validateSupersession(record, indexes, issues) {
  for (const id of record.supersedes) {
    const earlier = indexes.implementationById.get(id);
  ...
  if ((record.status === "SUPERSEDED") !== indexes.supersededIds.has(record.id)) issues.push(`${record.id} status does not match the declared supersession graph.`);
}

function validateImplementationDecisions(decisions, decisionIndex, issues) {
  ...
  const indexes = {
    ...decisionIndex,
    implementationById: new Map(implementationEntries.map((record) => [record.id, record])),
    migrationIds: new Set(decisions.migrationSequence.map((record) => record.id)),
    supersededIds: new Set(implementationEntries.flatMap((record) => record.supersedes)),
  };
  for (const record of implementationEntries) {
    validateImplementationBindings(record, indexes, issues);
    validateSupersession(record, indexes, issues);
  }
}

function validateWorkflowStageSequence(workflow, issues) {
...
function tierRangeIs(tier, minimum, maximum) {
  return tier?.minimumApplicableSteps === minimum && tier?.maximumApplicableSteps === maximum;
}

function validateWorkflowTiers(records, issues) {
  const tiers = new Map(records.map((record) => [record.id, record]));
  if (!tierRangeIs(tiers.get("TIER_1"), 53, 53)) issues.push("TIER_1 must require all 53 steps.");
  if (!tierRangeIs(tiers.get("TIER_2"), 25, 35)) issues.push("TIER_2 must require 25 through 35 applicable steps.");
...
function validateConstructionWorkflow(decisions, issues) {
  validateWorkflowStageSequence(decisions.constructionWorkflow, issues);
  validateWorkflowTiers(decisions.constructionWorkflow.tiers, issues);
...
function validateDecisionSemantics(decisions, indexes, issues) {
  validateDecisionIdentities(decisions, issues);
  validateDecisionLinks(decisions, indexes, issues);
  validateImplementationDecisions(decisions, indexes, issues);
  validateConstructionWorkflow(decisions, issues);
}

function validateTokenDecisionReferences(tokens, decisionById, issues) {
...
function validateTokenOrdering(tokens, issues) {
...
function validateTextPairing(pairing, colours, policy, issues) {
  ...
  if (!TEXT_USAGE_CLASSES.has(foreground.usageClass)) issues.push(`${pairing.id} foreground is not approved for text.`);
  ...
  if (Math.abs(actual - pairing.measuredRatio) > policy.ratioTolerance) issues.push(`${pairing.id} measuredRatio is stale; recorded ${pairing.measuredRatio}, actual ${actual.toFixed(3)}.`);
  const minimum = pairing.minimumClass === "NORMAL_TEXT" ? policy.normalTextMinimumRatio : policy.largeTextMinimumRatio;
  if (actual + policy.ratioTolerance < minimum) issues.push(`${pairing.id} contrast ${actual.toFixed(3)} is below ${minimum}.`);
}

function validatePaletteReferences(palette, colours, issues) {
...
function validateTokenSemantics(tokens, decisionBytes, indexes, issues) {
  ...
  validateTokenDecisionReferences(tokens, indexes.sourceById, issues);
  validateTokenOrdering(tokens, issues);
  const colours = new Map(tokens.colours.map((record) => [record.id, record]));
  for (const pairing of tokens.approvedTextPairings) validateTextPairing(pairing, colours, tokens.contrastPolicy, issues);
  for (const palette of tokens.viewPalettes) validatePaletteReferences(palette, colours, issues);
}

function appendEvidenceReferences(references, claim, values) {
  if (!Array.isArray(values)) return;
  for (const reference of values) {
    if (!reference || typeof reference.path !== "string") continue;
    references.push({ claim, path: normalizePath(reference.path), sha256: reference.sha256 });
  }
}

function appendConstructionReferences(record, references) {
  for (const entry of record?.evidence?.constructionEvidence || []) {
    for (const [evidenceClass, values] of Object.entries(entry.evidenceRefs || {})) {
      appendEvidenceReferences(references, `construction ${entry.stepId} ${evidenceClass}`, values);
    }
  }
}
...
function assetEvidenceReferences(record) {
  const references = [];
  appendConstructionReferences(record, references);
  for (const entry of record?.evidence?.proofEvidence || []) {
    const values = entry.evidenceRefs;
    appendEvidenceReferences(references, `proof ${entry.proofId}`, values);
  }
  return references;
}

function assetValidationContext(decisions, indexes, options) {
  const tracked = (options.trackedPaths || []).map(normalizePath);
  return {
    ...indexes,
    expectedStepIds: decisions.constructionWorkflow.steps.map((step) => step.id),
    componentBindings: new Map(Object.entries(options.componentBindings || {})),
    evidenceBindings: new Map(Object.entries(options.evidenceBindings || {})),
    fileHashes: new Map(Object.entries(options.fileHashes || {}).map(([file, hash]) => [normalizePath(file), hash])),
    releasePaths: new Set((options.releasePaths || []).map(normalizePath)),
    featureIds: new Set(options.featureIds || []),
    tutorialRefs: new Set(options.tutorialRefs || []),
    tracked,
    trackedSet: new Set(tracked),
  };
}

function validateAssetDecisionReferences(record, decisionsById, issues) {
...
function validateAssetRuleReferences(record, rulesById, issues) {
  for (const id of record.designRuleIds) {
    const rule = rulesById.get(id);
    if (!rule) issues.push(`${record.id} references unknown design rule ${id}.`);
    else {
...
function validateAssetTierCount(record, count, issues) {
  if (record.workflowTier === "TIER_1" && count !== 53) issues.push(`${record.id} TIER_1 must apply all 53 construction steps.`);
  if (record.workflowTier === "TIER_2" && (count < 25 || count > 35)) issues.push(`${record.id} TIER_2 must apply 25 through 35 construction steps.`);
  if (record.workflowTier === "TIER_3" && (count < 13 || count > 24)) issues.push(`${record.id} TIER_3 must apply 13 through 24 construction steps.`);
}

function validateAssetConstruction(record, context, issues) {
  const constructionEvidence = record.evidence.constructionEvidence;
  const { expectedStepIds, workflowStepsById } = context;
  ...
  validateAssetTierCount(record, applicableEntries.length, issues);
...
function validateAssetProofEvidence(record, issues) {
...
function validateAssetEvidenceFiles(record, context, issues) {
  const { trackedSet, fileHashes } = context;
...
function validateAssetRightsEvidence(record, componentBinding, evidenceBindings, issues) {
...
function validateAssetRights(record, context, issues) {
  const { componentBindings, evidenceBindings } = context;
  ...
  if (!componentBinding) issues.push(`${record.id} references unknown or ineligible file-bearing image component ${record.componentId}.`);
    ...
    validateAssetRightsEvidence(record, componentBinding, evidenceBindings, issues);
  }
}

function validateAssetBytes(record, context, issues) {
  const { trackedSet, fileHashes } = context;
...
function validateAssetMechanicReferences(record, context, issues) {
  const { featureIds, tutorialRefs } = context;
...
function validateMathematicalAssetProofs(record, issues) {
...
function validateAssetSemanticProofs(record, issues) {
  if (record.semanticClass === "MATHEMATICAL_REPRESENTATION") validateMathematicalAssetProofs(record, issues);
...
function validateCandidateAssetAcceptance(record, releasePaths, issues) {
...
function validateRuntimeAssetAcceptance(record, releasePaths, issues) {
  ...
  if (record.evidence.constructionEvidence.some((entry) => entry.status === "APPLICABLE_PENDING")) issues.push(`${record.id} runtime acceptance state has pending construction evidence.`);
...
function validateAssetRecord(record, context, issues) {
  ...
  validateAssetDecisionReferences(record, context.sourceById, issues);
  validateAssetRuleReferences(record, context.rulesById, issues);
  validateAssetConstruction(record, context, issues);
  validateAssetProofEvidence(record, issues);
  validateAssetEvidenceFiles(record, context, issues);
  validateAssetRights(record, context, issues);
  validateAssetBytes(record, context, issues);
  validateAssetMechanicReferences(record, context, issues);
  validateAssetSemanticProofs(record, issues);
  if (record.acceptanceState === "CANDIDATE") validateCandidateAssetAcceptance(record, context.releasePaths, issues);
  else validateRuntimeAssetAcceptance(record, context.releasePaths, issues);
}

function validateGovernedArtPaths(decisions, assets, tracked, paths, issues) {
...
function validateAssetSemantics(input, decisionBytes, indexes, issues) {
  const { decisions, assets, options } = input;
  ...
  const context = assetValidationContext(decisions, indexes, options);
  const paths = new Set();
  for (const record of assets.records) {
    ...
    validateAssetRecord(record, context, issues);
  }
  validateGovernedArtPaths(decisions, assets, context.tracked, paths, issues);
}

function validateGovernanceSemantics(input, issues) {
  const { decisions, tokens, options } = input;
  ...
  validateDecisionSemantics(decisions, indexes, issues);
  validateTokenSemantics(tokens, decisionBytes, indexes, issues);
  validateAssetSemantics(input, decisionBytes, indexes, issues);
}
...
export async function validateArtDesignGovernance(decisions, assets, tokens, options = {}) {
  ...
  validateGovernanceSemantics({ decisions, assets, tokens, options }, issues);
  return Object.freeze(issues);
}

async function readFileHash(repositoryRoot, file) {
  try {
    return sha256(await readFile(path.join(repositoryRoot, file)));
  } catch {
    return null;
  }
}

function imageComponentBindings(components) {
  return Object.fromEntries((components.bundledComponents || [])
...
async function loadEvidenceBindings(repositoryRoot, evidencePaths) {
  return Object.fromEntries(await Promise.all(evidencePaths.records.map(async (record) => [
    record.path,
    { actualSha256: await readFileHash(repositoryRoot, normalizePath(record.path)), sha256: record.sha256 },
  ])));
}

async function loadAssetFileHashes(repositoryRoot, assets) {
  ...
  return Object.fromEntries(await Promise.all([...governedFiles].map(async (file) => [
    file, await readFileHash(repositoryRoot, file),
  ])));
}

async function loadGovernanceRegistries(repositoryRoot) {
  ...
  return { components, evidencePaths, featureMap, releaseShell, tutorialManifest };
}

function governanceMemberships(options, registries) {
  const { featureMap, releaseShell, tutorialManifest } = registries;
  return {
    featureIds: options.featureIds || featureMap.features.map((record) => record.id),
    releasePaths: options.releasePaths || releaseShell.entries.map((record) => normalizePath(record.path)),
    tutorialRefs: options.tutorialRefs || [
...
async function resolveGovernanceBindings(options, repositoryRoot, assets) {
  const { componentBindings, evidenceBindings, fileHashes, featureIds, releasePaths, tutorialRefs } = options;
  if (componentBindings && evidenceBindings && fileHashes && featureIds && releasePaths && tutorialRefs) {
    return { componentBindings, evidenceBindings, fileHashes, featureIds, releasePaths, tutorialRefs };
  }
  const registries = await loadGovernanceRegistries(repositoryRoot);
  return {
    componentBindings: componentBindings || imageComponentBindings(registries.components),
    evidenceBindings: evidenceBindings || await loadEvidenceBindings(repositoryRoot, registries.evidencePaths),
    fileHashes: fileHashes || await loadAssetFileHashes(repositoryRoot, assets),
    ...governanceMemberships(options, registries),
  };
}
...
export async function loadArtDesignGovernance(options = {}) {
  ...
  const bindings = await resolveGovernanceBindings(options, repositoryRoot, assets);
