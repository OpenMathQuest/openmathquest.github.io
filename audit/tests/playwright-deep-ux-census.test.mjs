...
test("Playwright census uses native actionability, AI ARIA boxes, WebP anomaly capture, context network observation, and no pass artifacts", async () => {
  ...
  assert.equal((spec.match(/deepUxEffectBoundRerenderAction\(/gu) || []).length, 5);
  ...
  assert.match(spec, /expect\(exampleQuestionId\)\.not\.toBe\(sourceQuestionId\)/u);
  assert.match(spec, /data-answer-source/u);
  assert.match(spec, /PROCEDURE_ONLY_REQUIRED/u);
  assert.match(spec, /DIFFERENT_ANSWER_REQUIRED/u);
  assert.match(spec, /sourceTerminalAnswerFingerprint/u);
  assert.match(spec, /exampleTerminalAnswerFingerprint/u);
  ...
  assert.match(spec, /actualMathSurface/u);
  assert.match(spec, /describedByInstruction/u);
  assert.match(spec, /overlayHasSvgGeometry/u);
  assert.match(spec, /outlineWidth/u);
  assert.match(spec, /outlineStyle/u);
  assert.match(spec, /outlineOffset/u);
  assert.match(spec, /expectTutorialRenderedPhaseEffect\(tutorialPanel\)/u);
  assert.match(spec, /new Set\(tutorialPhaseSignatures\)\.size/u);
  ...
  assert.match(index, /lab\.tutorialStep\+=1/u);
