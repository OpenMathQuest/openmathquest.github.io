...
function validEvidence() {
  return {
    ...
    cacheProof: {
      physicalCacheName: `math-quest-static-v1.0.0-beta.6-${sha("c")}`,
    ...
    offlineProof: {
      ...
      readinessRelease: "1.0.0-beta.6",
      readinessBuildId: "math-quest-pwa-v1.0.0-beta.6",
      readinessCacheIdentity: "math-quest-static-v1.0.0-beta.6",
    },
    navigationProof: {
...
test("canary activates updates directly on Home and opens installation help only through the grown-up path", async () => {
  ...
  await waitForCanaryHomeUpdate(page, "1.0.0-beta.6", 25);
  assert.equal(state.requestedVersion, "1.0.0-beta.6");
...
test("canary deliberately reloads the existing Beta 1 page into the Home candidate", async () => {
  ...
  const candidate = await reloadCanaryCandidateFromBeta1(page, "1.0.0-beta.6", 75);
  assert.equal(candidate, page);
  assert.deepEqual(observations, [
    ["reload", "domcontentloaded", 75],
    ["version", "1.0.0-beta.6", 75],
...
test("candidate cache observation retries a concurrent install and binds one atomic snapshot", async () => {
  const expectedCacheName = "math-quest-static-v1.0.0-beta.6-digest";
...
test("canary checks emit progress markers and bind open-ended waits", async () => {
  ...
  assert.match(runnerText, /waitForCanaryHomeUpdate\(candidatePage, "1\.0\.0-beta\.6"\)/u);
  assert.match(runnerText, /activateCanaryHomeUpdate\(candidatePage\)/u);
  assert.match(runnerText, /reloadCanaryCandidateFromBeta1\(beta1Page, "1\.0\.0-beta\.6"\)/u);
