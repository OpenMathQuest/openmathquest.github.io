...
function parityMethod(question) {
...
function multiplicationMethod(question) {
  return ["partition", "array", "written layout"][Number(question.ordinal) % 3] ?? null;
}
...
export function strategyResultOracle(question) {
  if (["MQ-040", "MQ-041", "MQ-079", "MQ-095"].includes(question.skillId)) return arithmeticResult(question);
  if (question.skillId === "MQ-097") return parityResult(question);
  if (question.skillId === "MQ-101") return missingPartResult(question);
  assert.fail(`${question.skillId}: no strategy result oracle`);
}

function writtenAddition(top, bottom, width) {
  const work = [];
  let carry = 0;
...
function writtenSubtraction(top, bottom, width) {
  const work = [];
  const working = [...top];
  for (let index = width - 1; index >= 0; index -= 1) {
    let regroup = "no";
...
function writtenWork(left, right, operator) {
  ...
  return operator === "+" ? writtenAddition(top, bottom, width) : writtenSubtraction(top, bottom, width);
}

function additionWork(question, strategy) {
  const { left, right } = integers(question);
...
function subtractionWork(question, strategy, result) {
  const { left, right } = integers(question);
...
function multiplicationWork(question, strategy) {
  const { left, right } = integers(question);
...
function selectedArithmeticWork(question, strategy) {
  const { left, right } = integers(question);
...
export function strategyWorkOracle(question, selectedMethod = null) {
  ...
  if (question.skillId === "MQ-040") return additionWork(question, strategy);
  if (question.skillId === "MQ-041") return subtractionWork(question, strategy, result);
  if (question.skillId === "MQ-079") return multiplicationWork(question, strategy);
  if (question.skillId === "MQ-095") return selectedArithmeticWork(question, strategy);
  if (question.skillId === "MQ-097") return parityWork(question);
